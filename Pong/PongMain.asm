...
;--------------------------------------------------------------------------------------------
; INSTRUCTIONS:
; Left player uses keys 1 and 2
; Right player uses keys 9 and 0
;--------------------------------------------------------------------------------------------
...
ClearLP proc
	...
	mov dl, LPaddle.XCoord						;x coordinate is 1
...
DrawLP proc
	...
	xor edx, edx
	mov dl, LPaddle.XCoord						;Set the cursor x coord
	mov dh, LPaddle.YCoord						;Set the cursor y coord
	mov ecx, PaddleLength + 1					;Loop through
	;Write the paddle to the screen
	L1:
		call gotoxy
		mov al, "]"
		call WriteChar
		inc dh
	Loop L1
	call SetColorBW								;Set the color back to black and white
	call SetCursorToRead						;Set the cursor back to the bottom
	popad
	ret
DrawLP endp

;See ClearLP for comments --> exact same thing but with right paddle
ClearRP proc
	pushad
	xor eax, eax
...
;See Draw LP for comments --> exact same thing but with right paddle
DrawRP proc
	pushad
	xor eax, eax
...
DrawScore proc
	pushad
	; Should end the game once a player reaches the MaxScore
	.IF(P2Score == MaxScore || P1Score == MaxScore)
		mov EndGame, 1							;set the flag for ending the game
	.ENDIF
	xor edx, edx
	mov dl, RightBorder
	add dl, 5
	...
	;push the scores as arguments
	movzx eax, P1Score
	movzx ebx, P2Score
	push ebx
	push eax
	call Concat									;print a string to the screen
	call SetCursorToRead						;set the cursor to the bottom
	pop eax
	pop ebx
	popad
	ret
...
BallMath proc
	...
	.IF (TheBall.XCoord <= al)
		...
		.IF (al >= 1) && (al <= PaddleLength - 1)	;if it's at the end of either side of the paddle, make the run of the ball 2
			mov TheBall.Run, 2
		.ELSEIF (al == 0) || (al == PaddleLength)	;otherwise, make the run of the ball 1
			mov TheBall.Run, 1
		.ENDIF
	.ENDIF
	...
	;set the y coordinate of the ball given the rise
	mov al, TheBall.YCoord
	add al, TheBall.Rise
	mov TheBall.YCoord, al

	;set the x coordinate of the ball given the run
...
DrawBall proc
	...
	;figure out where the ball should go and write the ball to the screen
	call BallMath
	mov dl, TheBall.XCoord
	mov dh, TheBall.YCoord
	call gotoxy
...
;Logic for the instructions put into the queue
PaddleLogic proc
	pushad
	xor ecx, ecx
	...
	.IF (Queue.Countq > 0)
		...
		L1:
			call IQPopFront
			.IF (eax == 49)						;ascii value for 1
				call ClearLP
				inc LPaddle.YCoord
				call DrawLP
			.ELSEIF (eax == 50)					;ascii value for 2
				call ClearLP
				dec LPaddle.YCoord
				call DrawLP
			.ELSEIF (eax == 57)					;ascii value for 9
				call ClearRP
				inc RPaddle.YCoord
				call DrawRP
			.ELSEIF (eax == 48)					;ascii value for 0
				call ClearRP
				dec RPaddle.YCoord
				call DrawRP
			.ENDIF
...
PongMain proc C
	...
	;loop infinitely until game is over
	L1:
		call DrawBall
		call PaddleLogic
		call DrawScore
		xor eax, eax
		mov ecx, 12500			;length of the refresh cycle
		L2:
			push ecx
			call _kbhit			;listen for keyboard input
			.IF (eax != 0)		;if a key was hit
				call getch		;get that key ascii code
				push eax		;pass the ascii code as an argument
				call IQPushBack	;put that instruction into the queue of instructions
			.ENDIF
			pop ecx				;keep looping infinitely
		Loop L2
		.IF (EndGame == 1)		;This will end the game and exit the loop for now
			jmp GameOver		;end the game
		.ENDIF
	Loop L1
